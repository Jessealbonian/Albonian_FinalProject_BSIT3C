...
class StudentReg(models.Model):
    ...
    image = models.ImageField(upload_to="student-imgs/", blank=True, null=True, validators=[validate_image])
...
# Signal to delete the image file when the CustomerReg is deleted
@receiver(post_delete, sender=StudentReg)
def delete_image_on_profile_delete(sender, instance, **kwargs):
    if instance.image:
        if os.path.isfile(instance.image.path):
            os.remove(instance.image.path)

# Signal to delete the old image file when the CustomerReg image is updated
@receiver(pre_save, sender=StudentReg)
def delete_old_image_on_profile_update(sender, instance, **kwargs):
    if not instance.pk:
        return  # Skip if the instance is new

    try:
        old_instance = StudentReg.objects.get(pk=instance.pk)
